...
variable {m : Type u → Type v} {n : Type u → Type w} [Monad m] [Monad n]
  [LawfulMonad m] [LawfulMonad n] (F : m →ᵐ n)
...
theorem mmap_forIn' {ι : Type x} {β : Type u} (l : List ι) (init : β)
  ...
  F.isMonadHom.map_listForIn' l init f
...
theorem mmap_forM {ι : Type x} (l : List ι) (f : ι → m PUnit) :
  ...
  simpa only [List.forM_eq_forM, Function.comp_def] using F.isMonadHom.map_listForM l f

@[simp, grind =]
theorem mmap_foldlM {σ : Type u} {ι : Type x} (f : σ → ι → m σ) (init : σ) (l : List ι) :
    F (l.foldlM f init) = l.foldlM (fun s a => F (f s a)) init :=
  F.isMonadHom.map_listFoldlM f init l

@[simp, grind =]
theorem mmap_mapM {ι : Type x} {β : Type u} (f : ι → m β) (l : List ι) :
    F (l.mapM f) = l.mapM fun a => F (f a) :=
  F.isMonadHom.map_listMapM f l
...
theorem mmap_forIn_of_pureForIn {ρ : Type y} {ι : Type x} {β : Type u}
  ...
  F.isMonadHom.map_forIn_of_pureForIn xs init f
