...
supports `ipure` at diagonal indices and `ibind` that chains indices.
...
class LawfulIndexedMonad (I : Type*) (M : I → I → Type u → Type v)
  ...
  /-- Left identity. -/
  ipure_ibind {α β : Type u} {i j : I} (a : α) (f : α → M i j β) :
    ibind (ipure a) f = f a
  /-- Right identity. -/
  ibind_ipure {α : Type u} {i j : I} (x : M i j α) :
    ibind x ipure = x
  /-- Associativity. -/
...
export LawfulIndexedMonad (ipure_ibind ibind_ipure ibind_assoc)

attribute [simp] ipure_ibind ibind_ipure ibind_assoc

/-! ## Trivial indexing: every monad is an indexed monad over `Unit` -/

/-- Every `Monad` is an `IndexedMonad` over `Unit`,
with `ipure = pure` and `ibind = bind`. -/
...
instance instLawfulIndexedMonadOfLawfulMonad (m : Type u → Type v)
  ...
  ipure_ibind a f := by simp [ipure, ibind]
  ibind_ipure x := by simp [ipure, ibind]
