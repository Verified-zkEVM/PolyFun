...
@[simp]
theorem someOutput_map (p : β → Prop) (f : α → β) (x : m α) :
    SomeOutput p (f <$> x) ↔ SomeOutput (p ∘ f) x := by
  change (∃ a ∈ support (f <$> x), p a) ↔ ∃ a ∈ support x, p (f a)
  simp only [support_map, Set.exists_mem_image]

@[simp]
theorem noOutput_map (p : β → Prop) (f : α → β) (x : m α) :
    NoOutput p (f <$> x) ↔ NoOutput (p ∘ f) x :=
  allOutputs_map (fun b => ¬ p b) f x

@[simp]
theorem allOutputs_seq (p : β → Prop) (f : m (α → β)) (x : m α) :
    AllOutputs p (f <*> x) ↔ AllOutputs (fun g => AllOutputs (p ∘ g) x) f := by
  rw [seq_eq_bind_map, allOutputs_bind]
  simp only [allOutputs_map]
  exact Iff.rfl

@[simp]
theorem someOutput_seq (p : β → Prop) (f : m (α → β)) (x : m α) :
    SomeOutput p (f <*> x) ↔ SomeOutput (fun g => SomeOutput (p ∘ g) x) f := by
  rw [seq_eq_bind_map, someOutput_bind]
  simp only [someOutput_map]
  exact Iff.rfl
...
@[simp]
theorem allOutputs_seqLeft (p : α → Prop) (x : m α) (y : m β) :
...
@[simp]
theorem allOutputs_seqRight (p : β → Prop) (x : m α) (y : m β) :
...
@[simp]
theorem someOutput_seqLeft (p : α → Prop) (x : m α) (y : m β) :
...
@[simp]
theorem someOutput_seqRight (p : β → Prop) (x : m α) (y : m β) :
