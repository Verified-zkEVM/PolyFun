...
namespace Interaction.Execution.RequestNetwork
...
@[expose] def State.rename (e : Client ≃ Client') (state : State Client p α S) :
    State Client' p α S where
...
variable [DecidableEq Client] [DecidableEq Client'] [DecidableEq p.A]

omit [DecidableEq p.A] in
private theorem update_rename (e : Client ≃ Client') (clients : Client → ClientState p α)
    (id : Client) (next : ClientState p α) :
...
theorem deliver_rename (impl : Handler (StateT S m) p) (e : Client ≃ Client')
    (state : State Client p α S) :
...
theorem step_rename (impl : Handler (StateT S m) p) (e : Client ≃ Client')
...
theorem run_rename (impl : Handler (StateT S m) p) (e : Client ≃ Client')
    (schedule : List (Activation Client)) (state : State Client p α S) :
...
end Interaction.Execution.RequestNetwork
