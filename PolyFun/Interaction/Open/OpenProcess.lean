...
The structural data has four layers:
...
* `OpenProcess m Party Δ` adds an intrinsic `m`-valued sampler at each step.
...
closed `Process` through `OpenProcess.toProcess`.
...
decorated step path. Routing and buffering are interpreted by the generic
`Interaction.Execution` layer; probabilistic execution is supplied downstream.
-/
