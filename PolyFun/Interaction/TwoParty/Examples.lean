...
example : Spec.StrategyOver (TwoParty.pairedSyntax m) TwoParty.Participant.focal
    (exSpec T U) (exRoles T U) (fun _ => α)
    = m ((_ : T) × ((_ : U) → m α)) := rfl

example : Spec.StrategyOver (TwoParty.pairedSyntax m) TwoParty.Participant.counterpart
    (exSpec T U) (exRoles T U) (fun _ => α)
    = ((_ : T) → m (m ((_ : U) × α))) := rfl
