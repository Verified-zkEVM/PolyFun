...
transition lens `Lens.fixState` (Ex 6.44, `δ = (id, tgt, run)`) and its counit
is the stay-put self-loop `s ↦ (⋆ ↦ s)`; this discharges the state-comonoid laws
of that transition lens. -/
