...
`simp` pushes a bundled morphism between lawful monads into the body of every loop combinator,
and `grind` closes goals that need the list forms as rewrite rules.
-/
...
example (F : m →ᵐ n) (l : List Nat) (f : Nat → m Nat) :
