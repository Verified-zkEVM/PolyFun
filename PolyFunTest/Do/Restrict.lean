...
The identity algebra of `PolyFunTest.Do.Algebra` restricted to `[0, 1] ⊆ ℕ∞` drives `vcgen` like any
other ordered algebra: the carrier's own bound is available as `le_top`, and the restricted `wp` is
the original one on underlying values. The module acknowledges the tactic's experimental status with
