...
example (P : PFunctor.{uA, uB}) (n : ℕ) : Lens (CofreeP P) (compNth P n) :=
  CofreeP.projectionN P n

example (P : PFunctor.{uA, uB}) (n : ℕ) (tree : M P)
    (direction : (compNth P n).B ((CofreeP.projectionN P n).toFunA tree)) :
    M.Vertex.depth ((CofreeP.projectionN P n).toFunB tree direction) = n :=
...
example : (CofreeP.projectionN binaryP 0).toFunB binaryTree PUnit.unit = .root binaryTree :=
  rfl
...
example : (CofreeP.projectionN binaryP 1).toFunB binaryTree ⟨false, PUnit.unit⟩ =
    .child false (.root (M.children binaryTree false)) :=
  rfl

/-- Two distinct directions are retained in outer-then-inner order. -/
example : (CofreeP.projectionN binaryP 2).toFunB binaryTree ⟨false, ⟨true, PUnit.unit⟩⟩ =
    .child false (.child true (.root (M.children (M.children binaryTree false) true))) :=
  rfl

/-- The concrete two-edge pullback has exactly depth two. -/
example : M.Vertex.depth ((CofreeP.projectionN binaryP 2).toFunB binaryTree
      ⟨false, ⟨true, PUnit.unit⟩⟩) = 2 :=
  CofreeP.depth_projectionN_toFunB binaryP 2 binaryTree ⟨false, ⟨true, PUnit.unit⟩⟩
