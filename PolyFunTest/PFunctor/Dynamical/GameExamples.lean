...
deterministic PrivK-shaped `game₂` (challenger via `Lens.ofCompTriple`,
...
The challenger is built from its destructor triple (`Lens.ofCompTriple`), and
...
def privKChallenger :
  ...
  Lens.ofCompTriple
    ⟨fun b => sectionLens (fun mm => cond b mm.2 mm.1),
      fun b _ => (fun (guess : Bool) => guess == b) ⇆ (fun _ _ => PUnit.unit),
      fun b _ => b⟩
