...
example : directionVal false false
    (Responder.appD contract verifiedBehavior query false).1 = 1 := by
  change directionVal false false
    (Responder.appD contract
      (Responder.verifiedBehavior contract responder Invariant verified
        false initialWitness) query false).1 = 1
  rw [Responder.appD_verifiedBehavior_post]
  rfl
...
example : directionVal true false
    (Responder.appD contract verifiedBehavior query true).1 = 1 := by
...
example :
    Display.M.transport (Responder.behavior_child responder false ())
        (Responder.appD contract verifiedBehavior query false).2 =
      Responder.verifiedBehavior contract responder Invariant verified
  ...
  exact
    Responder.appD_verifiedBehavior_next contract responder Invariant verified
...
/-! The direct displayed execution and the coinductive reindexed `appD` side
...
example :
    Display.M.transport
        ...
        (Responder.appD contract reindexedVerifiedBehavior query false).2 =
      Responder.reindexVerifiedBehavior contract contract oneCall
        displayedOneCall
        ((Responder.terminal (P := Interface)).runFree
          (oneCall query) behavior).2 directDisplayedStep.2 := by
  exact Responder.appD_reindexVerifiedBehavior_next contract contract
...
example : directionVal true true
    (Responder.appD contract firstReindexedContinuation query true).1 = 0 := by
  change directionVal true true
    (Responder.appD contract
      ...
      query true).1 = 0
  rw [Responder.appD_reindexVerifiedBehavior_next]
  rfl
...
/-! This unfolds the continuation returned by the first `appD` call and then
observes its next answer-dependent postcondition. It is independent of the
one-step continuation equality above. -/
example : directionVal true true
    (Responder.appD contract firstVerifiedContinuation query true).1 = 0 := by
  change directionVal true true
    (Responder.appD contract
      (Display.M.transport (Responder.behavior_child responder false query)
        (Responder.appD contract
  ...
  rw [Responder.appD_verifiedBehavior_next]
  rw [Responder.appD_verifiedBehavior_post]
  rfl
...
universe uI uA uB uC uD uX uS uF
