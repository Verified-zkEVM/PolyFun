...
- `Cslib.Turing.SingleTapeTM.tableComputer`: read the input into machine state through the
  ...
  bound (a uniform description bound) only on domains of polynomially bounded
  cardinality. Within that regime it subsumes constants, relabelings, and projections,
  and discharges all four per-step machine witnesses for
...
private lemma tableComputer_finish_within (S : Finset (List Symbol))
  ...
  · by_cases hT : (T l).length = 0
    · rw [dite_eq_left hS, dite_eq_left hT] at h1
      ...
      exact RelatesWithinSteps.mono (by omega) (.single h1)
    · rw [dite_eq_left hS, dite_eq_right hT] at h1
      ...
      refine RelatesWithinSteps.mono ?_
        ((RelatesWithinSteps.single h1).trans
          (RelatesWithinSteps.of_relatesInSteps h2))
...
of parameter-indexed backend certificates. -/
...
a uniform description bound fields. -/
