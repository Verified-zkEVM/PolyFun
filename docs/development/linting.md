...
`PolyFun`, so `lake exe lint-style` alone misses the auxiliary roots and tutorials. The style CI job
...
- `lake exe lint-style PolyFun ToCslib ComplexityBackends Examples.Tutorials.Requests Examples.Tutorials.Machines Examples.Tutorials.IndexedPrograms Examples.Tutorials.InteractionTrees` uses Mathlib's source-text checks,
...
[notation.md](../reference/notation.md) for operand order and the relation to book notation.

Follow the linter policy in [AGENTS.md](../../AGENTS.md#lean-and-api-policy).
