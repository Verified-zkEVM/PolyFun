...
### `autoImplicit = false` is set globally in `lakefile.toml`
...
### `TypeTree.done` and `TypeTree.node` are `@[match_pattern, reducible]` wrappers
...
### Do not disable linters to silence errors
...
### Core types are `@[reducible]` thin wrappers
...
Upstream simplification presents `liftBind a k` as `(FreeM.lift a).bind k`, or
`FreeM.lift a >>= k` when the universes agree. Prefer compositional pure/bind/lift
laws to node-specific normalization helpers. For a structural proof, select the
constructor equation with `rw` and use `change` when its input is presented as a
bind. Do not change the upstream simp attributes to suit the proof.

A dependent type such as `P.B a` can reduce on a concrete signature and prevent
simp from finding an otherwise applicable lemma. Try `rw [the_named_equation]`;
a lookup miss alone does not justify new syntax or another equation variant.
Standard `no_index` annotations on individual dependent type arguments are
appropriate for genuine public simp laws, with a concrete regression test.

When matching path or displayed-family indices needs bind and lift to unfold at
implicit transparency, use a local `implicit_reducible` attribute on those
definitions. Keep the scope limited to the module requiring that equality.
See [the free-monad guide](../guides/pfunctor.md#where-to-start).
...
### `do`-notation goals take the standard monad laws directly
...
### A predicate whose leading argument is implicit cannot be passed as an argument
...
argument in
[`PolyFun/Interaction/Open/SubTheory.lean`](../../PolyFun/Interaction/Open/SubTheory.lean).
...
### `@[expose]` defs, or downstream modules cannot unfold them
...
### Pattern-matching lambdas in statements are declaration-local
...
### Keep one canonical concrete-step relation type
...
### Alias layers over generic types: shadow the chained operations
...
### Alias layers: alias-namespace lemmas are not dot-callable on generic-headed values
...
### Avoid `cast` / `Eq.mp` / `Eq.mpr` in core definitions
...
### Preserve partial proof attempts with `stop`
...
### Prefer existing combinators over bespoke wrappers
...
### Module organization: no thin re-export umbrellas inside subdirectories
...
**Allowed umbrellas** are the top-level library roots: `PolyFun.lean`,
`ToCslib.lean`, and `ComplexityBackends.lean`, all generated by
[`scripts/update-lib.sh`](../../scripts/update-lib.sh); see
[`generated-files.md`](generated-files.md). A backend subdirectory under
`ComplexityBackends/` gets no umbrella of its own.
...
[`AGENTS.md`](../../AGENTS.md), the documentation, and any other guidance files.
...
### Fetch the dependency cache on a fresh checkout

Fetch the precompiled dependency cache before the first build or after a
dependency update. Routine builds can reuse the existing cache.
...
### Lean toolchain and Mathlib version must stay in sync
...
### Use public references in shared docs
...
### Projections re-synthesize instance-implicit structure arguments
...
### `grind =` cannot index `ite`, `dite`, or thunked applicative operands
...
### A `Set` literal ascribed as `SetM` is still a `Set`
...
### `vcgen` matches specs structurally, so dependent value types need care
