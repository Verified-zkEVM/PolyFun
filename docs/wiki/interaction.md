...
3. **`openTheory Party m schedulerSampler` threads samplers
   compositionally.**
...
| `OpenSyntax/Expr.lean` | `Expr` (quotient of `Raw`), `Expr.theory` as a reducible `Expr` facade definitionally equal to `(Raw.theory Atom).quotient (Raw.congruence Atom)`, with its named law instances sharing the generic lifting, the `theory_{map,par,wire,plug,unit,idWire}` computation lemmas, `Expr.toInterp` |
...
| `OpenProcessQuotient.lean` | `openTheory.activationCongruence` (activation equivalence as a congruence; the laws modulo it are the `openTheory_*_activation_equiv` theorems, so the activation quotient is strictly `HasPlugWireFactor`), `openTheory.samplerCongruence R` (lawful monad and `R.IsBindCongr`) with `hasPlugFactorization_quotient_samplerCongruence` under the transport facts, `scheduledOpenTheory.samplerCongruence R` (equal mass and sampler-equivalent processes, with the same monad/relation requirements) with `hasPlugFactorization_quotient_samplerCongruence` under scheduler coherence, and `Observation.activation` / `Observation.sampler` as pull-backs of equality on the quotients |
...
| `MomentaryCorruption.lean` | compromise/refresh bookkeeping over an abstract identity `M`, with reactions parametric over `m` |
| `Leakage.lean` | per-party projection interface; observation semantics and security laws are supplied downstream |
