...
| `SamplerObservation.lean` | `Observation.respectsFactorization_of_samplerInvariant` — any observation invariant under sampler equivalence respects plug commutation and factorization, given the transport facts — and the canonical packet- and sampler-aware observation `Observation.sampler`. Together with the gate map in `docs/wiki/uc.md` this is the bridge a downstream distributional observation instantiates. |
