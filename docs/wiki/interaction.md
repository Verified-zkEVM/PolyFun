...
| `GlobalSubroutine.lean` | `OpenTheory.withGlobal` (wire a shared global resource onto a protocol's subroutine face), `EmulatesWithGlobal` / `EmulatesWithGlobalWithin` (FKKKT26 Definition III.13 in wire-composed form), and the UCGS-style composition theorems `wire_outer` / `wire_compose_outer` (Theorem III.14), all derived from the existing wire composition suite. |
| `SecureEmulation.lean` | `SecurelyEmulates` / `SecurelyEmulatesWithin`, the existential-simulator (directional) emulation judgment, its reconciliation with `Emulates` and `UCSecure`, and `securelyEmulatesPreorder` (FKKKT26 Definition A.1). No `par`/`wire` monotonicity: that needs structural simulators and compact closure, which the process model does not strictly possess. |
| `DummyAdversary.lean` | `emulates_wire_dummy_iff` and `Emulates.of_wire_splitMono` (FKKKT26 Theorem III.7 / Corollary III.9 / Lemma IV.3 in wired form): checking emulation against the dummy or any split-mono adversary suffices in `IsCompactClosed` theories. Not available for `openTheory`, whose zig-zag laws hold only up to activation equivalence. |
