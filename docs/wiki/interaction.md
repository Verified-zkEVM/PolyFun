...
| `Scheduler.lean` | `ScheduleMass`, mass-aware `BinaryScheduler`, hierarchical source/left/right draws, and `BinaryScheduler.IsFlat` / `IsCoherent`. `IsFlat` requires every binary encoding of a three-component choice to agree with one direct flat choice relative to a downstream `MonadRelFamily`. |
