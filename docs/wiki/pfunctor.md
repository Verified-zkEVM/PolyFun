...
| File | Purpose |
|------|---------|
| [`PolyFun/PFunctor/Basic.lean`](../../PolyFun/PFunctor/Basic.lean) | `PFunctor` core, `Obj`, the identity polynomial `y`, monomials `A y^ B` / representables `y^ B`, sum / product / sigma / pi (`Σₚ` / `Πₚ`) / tensor / composition (`◃`, powers `◃^`), `Lens`, `selfMonomial`, ring-style `0` / `1` / `+` / `*`. |
| [`PolyFun/PFunctor/Deprecated.lean`](../../PolyFun/PFunctor/Deprecated.lean) | Transitional compatibility surface for the former `X`-based spelling: deprecated aliases onto the `y`-named declarations, a parse-only `A X^ B` notation, and the compatibility `NatPow` instance. Slated for removal once downstream projects migrate. |
