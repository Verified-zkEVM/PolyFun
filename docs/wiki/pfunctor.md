...
| [`PolyFun/PFunctor/Dynamical/Bisimulation.lean`](../../PolyFun/PFunctor/Dynamical/Bisimulation.lean) | Bridge to the generic `Control.WeakBisim` / `Control.StrongBisim` theory: `DynSystem.toLTS` exhibits a `p`-system as a `Control.LTS`, and `obsEq_of_isStrongBisim` proves a generic strong bisimulation on the induced systems forces `ObsEq` (behaviour-tree equality) via `M.corec_eq_corec`. See [`bisimulation.md`](bisimulation.md). |
