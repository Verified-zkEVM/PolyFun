...
| [`PolyFun/PFunctor/Dynamical/PointedMachine.lean`](../../PolyFun/PFunctor/Dynamical/PointedMachine.lean) | `PointedMachine`: pointed machines (`init` / partial `output`) over an interface; `seqComp` (⊕-state sequential composition, Ex 6.41) with `toComp_seqComp_inl/inr`; fuelled `toComp` with `isTotalRollBound_toComp`; monad-parametric `runWith` = `mapM ∘ toComp` with `runWith_succ` / `runWith_of_output_eq_some`. |
