...
| File | Purpose |
|------|---------|
| [`PolyFun/PFunctor/Basic.lean`](../../PolyFun/PFunctor/Basic.lean) | `PFunctor` core, `Obj`, the identity polynomial `y`, monomials `A y^ B` / representables `y^ B`, sum / product / sigma / pi (`Σₚ` / `Πₚ`) / tensor / composition (`◃`, powers `◃^`), `Lens`, `selfMonomial`, ring-style `0` / `1` / `+` / `*`; also the foundational deprecated `X`/`X^` and `NatPow` compatibility needed by direct importers. |
| [`PolyFun/PFunctor/Deprecated.lean`](../../PolyFun/PFunctor/Deprecated.lean) | Aggregate import for the former `X`-based compatibility surface. Each deprecated alias lives beside its canonical declaration so direct module imports remain compatible. Slated for removal once downstream projects migrate. |
...
| [`PolyFun/PFunctor/InternalHom.lean`](../../PolyFun/PFunctor/InternalHom.lean) | The `⊗`-internal hom `ihom q r` = `[q, r]` (Ex 4.78; positions are lenses `q ⇆ r`), the evaluation lens `Lens.eval`, and the tensor–hom adjunction `curry` / `uncurry` / `curryEquiv : Lens (p ⊗ q) r ≃ Lens p (ihom q r)`; `ihomSum`, `ihomY`. |
...
| [`PolyFun/PFunctor/Handler.lean`](../../PolyFun/PFunctor/Handler.lean) | `Handler m p`: a monadic choice of direction at every position of `p`; the generic interface consumed by `FreeM.liftM`, with indexed coproduct combination and pointwise target change through `Handler.mapTarget`. |
| [`PolyFun/PFunctor/Handler/Instrumentation.lean`](../../PolyFun/PFunctor/Handler/Instrumentation.lean) | Target-monad lifting, pre- and post-handler effect insertion, and response-independent or response-dependent writer traces. |
...
| [`PolyFun/PFunctor/Handler/Stateful.lean`](../../PolyFun/PFunctor/Handler/Stateful.lean) | `Handler.Stateful m S p`, the transparent `Handler (StateT S m) p` effectful Mealy interface; finite-program execution, stateless lifting, and contravariant reindexing by free handlers with identity, composition, and fusion laws. |
| [`PolyFun/PFunctor/Resumption.lean`](../../PolyFun/PFunctor/Resumption.lean) | `Resumption p β := M (p + C β)`, the operation-first canonical possibly infinite tau-free return-or-query behavior; computational-view coinduction via `HeadMatch` / `bisim` / `corec_unique`; universe-polymorphic `map` / `bind` and lawful monad structure; and interface transport by `mapLens` with identity, composition, bind, and map laws. |
| [`PolyFun/PFunctor/Resumption/Empty.lean`](../../PolyFun/PFunctor/Resumption/Empty.lean) | Exact equivalence `M P ≃ Resumption P PEmpty`, eliminating the impossible return summand, with the visible-query destructor equation. |
| [`PolyFun/PFunctor/Resumption/WellFounded.lean`](../../PolyFun/PFunctor/Resumption/WellFounded.lean) | Exact fixed-point bridge `FreeM P α ≃ {r : Resumption P α // r.WellFounded}`, including the commuting W/M factorization and an iff characterization of the `FreeM.toResumption` image. |
| [`PolyFun/PFunctor/Resumption/Truncate.lean`](../../PolyFun/PFunctor/Resumption/Truncate.lean) | Fuelled truncation of a possibly infinite resumption into `FreeM p (Option β)`, with exact characterization by the well-founded-program embedding and total roll bounds. |
| [`PolyFun/PFunctor/Dynamical/DynComputation.lean`](../../PolyFun/PFunctor/Dynamical/DynComputation.lean) | `DynComputation p α β`, extending the operation-first `Machine (p + C β)` with `init`; `view` exposes returns or visible queries and `denote` gives `α → Resumption p β`. Input contravariance, result mapping, lens wrapping, and `dimap` have structural and denotational laws. Universe-independent `ObsEq` compares denotations across hidden-state representations and transports `Implements`. `seqComp` uses phase-sum hidden state with a query-free return handoff, denotes exact `Resumption.bind`, preserves implementation by `FreeM.bind`, and satisfies identity/associativity observationally. `ofResumption` / `ofFreeM` and synchronized simulation provide canonical realizations and qualitative correctness. |
| [`PolyFun/PFunctor/Dynamical/DynComputation/Bounded.lean`](../../PolyFun/PFunctor/Dynamical/DynComputation/Bounded.lean) | Executable fuelled execution where returns are free and each visible query costs one; `unroll_wrap` for interface transport; recursive branchwise `ResolvesIn`; exact bounded-implementation characterizations; additive sequential budgets under resolution certificates; and monadic-handler execution, including the query-conditioned `StateT` law consumed by games. For closed interface `y`, `closedStutterStep` / `closedStutterIterate` follow the unique query direction and explicitly stutter after return; bounded and eventual iff theorems identify resolution with reaching a returned trajectory state. |
| [`PolyFun/PFunctor/Dynamical/DynComputation/BoundedFold.lean`](../../PolyFun/PFunctor/Dynamical/DynComputation/BoundedFold.lean) | Countdown-state realization of fixed-query `FreeM.foldr` programs, including a branchwise roll bound and an exact `ImplementsWithin` bridge after mapping the final readout. |
| [`PolyFun/PFunctor/Dynamical/DynComputation/Termination.lean`](../../PolyFun/PFunctor/Dynamical/DynComputation/Termination.lean) | Accessibility-based qualitative termination of `DynComputation`, computable extraction of well-founded `FreeM` programs, exact semantic and resource characterizations, equivalence with `Resumption.WellFounded` of the state-free behavior, and closure under sequential composition. This is strictly more general than a uniform `ResolvesIn` budget for infinitely branching computations. |
| [`PolyFun/PFunctor/Dynamical/RunN.lean`](../../PolyFun/PFunctor/Dynamical/RunN.lean) | `DynSystem.twoStep` (Ex 6.44) and `DynSystem.nStep` = `Run_n(φ) = φ^{◁n} ∘ₗ δ^{(n)}` (§7.1.5) over `compNth p n`; `nStep_two_eq_twoStep` identifies the `n = 2` specialization after the inner unitor. The transition lens itself is `Lens.fixState`. |
...
| [`PolyFun/PFunctor/Dynamical/Responder.lean`](../../PolyFun/PFunctor/Dynamical/Responder.lean) | `Responder S q := DynSystem S (q ⊸ y)`: state systems whose positions are committed answer-sections (`committed` / `answer` / `next`); the Kleisli–Mealy `equivStateHandler : Responder S q ≃ Handler (StateT S Id) q` with `rfl` round-trips. |
| [`PolyFun/PFunctor/Dynamical/Game.lean`](../../PolyFun/PFunctor/Dynamical/Game.lean) | Game wiring: `DynSystem.game := wire₂ (Lens.eval q r)` and its autonomous `closedGame`, `game_eq_uncurry` (the adjunction reading); monadic runs `kleisliStep` / `kleisliIterate` / `stepWith` / `iterWith`; and two-phase `Lens.eval₂` / `orderPair` / `game₂` (Eq 6.86 / Ex 6.85). Returning-computation interaction uses `runWith_query_succ_stateT`, whose explicit query-view hypothesis prevents fabricating a transition from a terminal state. |
...
| [`PolyFun/ITree/Unfold.lean`](../../PolyFun/ITree/Unfold.lean) | `DynSystem.toITree`: unfold a dynamical system into an all-query interaction tree; `M.toITree` embeds behaviour trees and agrees with the route through `M.toEmptyResumption` and the tau-free resumption embedding. |
...
| [`PolyFun/PFunctor/Display/Handler.lean`](../../PolyFun/PFunctor/Display/Handler.lean) | Displayed handlers, free extension, Kleisli composition, hom-fiber transport, and transported identity/associativity laws. |
...
| [`PolyFun/PFunctor/Display/Lens.lean`](../../PolyFun/PFunctor/Display/Lens.lean) | Fiberwise morphisms between displays over a base polynomial lens; faithful total-polynomial semantics and the one-operation displayed-handler embedding. |
| [`PolyFun/PFunctor/Parallel.lean`](../../PolyFun/PFunctor/Parallel.lean) | One-or-both parallel composition `P ∥ Q`, with its coproduct/tensor decomposition, unit, symmetry, and associativity equivalences. |
| [`PolyFun/PFunctor/Display/Parallel.lean`](../../PolyFun/PFunctor/Display/Parallel.lean) | General unary-left/unary-right/joint displays over `P ∥ Q`, heterogeneous component assembly, and the separable product specialization. |
...
| [`PolyFun/PFunctor/Dynamical/Responder/Behavior.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Behavior.lean) | State-free responder behavior as the existing `M (P ⊸ y)`; coinductive `respondDisplayed` (the intrinsic form of Aberlé's `appD`) and observation-shaped bisimulation through `Display.M (Display.responder S)`; agreement of finite displayed execution with state presentations; and ordinary/displayed state-free handler reindexing with transported identity/composition and exact post/continuation equations. |
| [`PolyFun/PFunctor/Dynamical/Responder/Presentation.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Presentation.lean) | Proof-relevant homomorphisms between responder presentations: state and dependent-witness maps preserving the complete totalized displayed step, with identity, composition, terminal semantics, and state-free naturality. |
| [`PolyFun/PFunctor/Dynamical/Responder/Lens.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Lens.lean) | Ordinary and displayed state-free reindexing along polynomial/display lenses, including heterogeneous response universes and transported identity/composition laws. |
| [`PolyFun/PFunctor/Dynamical/Responder/Parallel.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Parallel.lean) | Coproduct and one-or-both parallel responders, with explicit component-state freezing and unit/symmetry/associativity equations. |
...
| [`PolyFun/PFunctor/Dynamical/Responder/Parallel/Behavior.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Parallel/Behavior.lean) | Ordinary and displayed state-free coproduct/parallel behavior, with exact answer, postcondition, and continuation equations. |
...
| [`PolyFun/PFunctor/Dynamical/Responder/Parallel/Presentation.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Parallel/Presentation.lean) | Componentwise parallel composition of displayed presentation homomorphisms, with identity and interchange laws. |
...
| [`PolyFun/PFunctor/Dynamical/Responder/Parallel/DisplayedAssociativity.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Parallel/DisplayedAssociativity.lean) | Proof-relevant associativity first at explicit displayed presentations and then at canonical state-free semantics. |
| [`PolyFun/PFunctor/Dynamical/Responder/Parallel/DisplayedCoherence.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Parallel/DisplayedCoherence.lean) | Proof-relevant symmetry and unit coherence, plus the canonical empty displayed behavior. |

The displayed responder layer uses structural names rather than verification
claims. `Display` is an arbitrary `Type`-valued polynomial family;
`Display.Coalgebra` preserves its one-step data over a responder;
`toDisplayedBehavior` coinduces that data over the responder's state-free
behavior; and `PresentationHom` preserves the complete step between two
state-and-witness presentations. If a particular display and witness family
encode a specification and invariant, an inhabitant can justify calling that
application verified. The generic objects can also carry traces, resources,
logical relations, or other annotations, so their API names do not make that
stronger claim.
...
| [`PolyFun/PFunctor/Free/Fold.lean`](../../PolyFun/PFunctor/Free/Fold.lean) | Fixed-query interaction folds `FreeM.foldl` and `FreeM.foldr`: each answer updates an accumulator at an increasing or decreasing natural-number index, a final readout produces the result, and map laws fuse postprocessing into that readout. |
| [`PolyFun/PFunctor/Free/Sigma.lean`](../../PolyFun/PFunctor/Free/Sigma.lean) | `FreeM.sigmaInj` injects one member of an arbitrary indexed family into the sigma interface while tagging returned values; `FreeM.packFamily` packages the whole dependent input/program family. This is syntactic packing, not a uniformity or complexity assertion. |
| [`PolyFun/PFunctor/Free/Support.lean`](../../PolyFun/PFunctor/Free/Support.lean) | Exact structural support of finite free programs and its `AllOutputs` / `SomeOutput` / `NoOutput` judgments. |
...
| [`PolyFun/PFunctor/Free/Cursor/ReplayTree.lean`](../../PolyFun/PFunctor/Free/Cursor/ReplayTree.lean) | Finite nested replay schedules with dependent leaf addresses, source-path reconstruction, output preservation, and shared-prefix traces. Sampling, sibling distinctness, acceptance, and extraction bounds are downstream obligations. |
...
| File | Purpose |
|------|---------|
| [`PolyFun/PFunctor/Cofree.lean`](../../PolyFun/PFunctor/Cofree.lean) | `CofreeC` (cofree comonad on a `PFunctor`), built from `PFunctor.M` of the `constProd` polynomial; `CofreeC.mapLens` supplies label-preserving transport with root/tail, identity, and composition laws. The dual of `FreeM`. |
