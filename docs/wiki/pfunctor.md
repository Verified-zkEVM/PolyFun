...
| [`PolyFun/PFunctor/Dynamical/RunN.lean`](../../PolyFun/PFunctor/Dynamical/RunN.lean) | `DynSystem.twoStep` (the `n = 2` composite over the binary `p ◃ p`, via `Lens.speedup` / the transition lens `Lens.fixState`) and the general `DynSystem.nStep` = `Run_n(φ) = δ^{(n)} ⨟ φ^{◁n}` (§7.1.5) over the composition power `compNth p n`; `twoStep_toLens_eq` collapses `n = 2` to `twoStep`. |
| [`PolyFun/PFunctor/Dynamical/Refinement.lean`](../../PolyFun/PFunctor/Dynamical/Refinement.lean) | Simulations and refinement between dynamical systems. Tight: `DynSystem.IsSimulation` (step-synchronized) with `implements_of_isSimulation` (equal `behavior` via `M.corec_eq_corec`) and the coalgebra-morphism instances `isSimulation_graph` / `behavior_coalgHom`. Lax: `DynSystem.ForwardSimulation` (existential refinement between `System`s over possibly different interfaces, step-matched by a `DirRel`) with `mapRun` and its transport lemmas, `BackwardSimulation` / `Bisimulation`, and `ForwardSimulation.ofIsSimulation` bridging the two at `DirRel.sync`. Instantiated by the concurrent `Refinement` / `Bisimulation` layers. |
