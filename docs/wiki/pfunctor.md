...
| [`PolyFun/PFunctor/Dynamical/Responder/Behavior.lean`](../../PolyFun/PFunctor/Dynamical/Responder/Behavior.lean) | State-free responder behavior as the existing `M (P ⊸ X)`; coinductive `appD` and observation-shaped bisimulation through `Display.M (Display.responder S)`; agreement of finite displayed execution with state presentations; and ordinary/verified state-free handler reindexing with transported identity/composition and exact post/continuation equations. |
