...
| [`PolyFun/PFunctor/Dynamical/DynComputation.lean`](../../PolyFun/PFunctor/Dynamical/DynComputation.lean) | `DynComputation p α β`, extending `Machine (C β + p)` with `init`; `view` exposes returns or visible queries and `denote` gives `α → Resumption p β`. Input contravariance, result mapping, lens wrapping, and `dimap` have structural and denotational laws. Universe-independent `ObsEq` compares denotations across hidden-state representations and transports `Implements`; `ofResumption` / `ofFreeM` and synchronized simulation provide canonical realizations and qualitative correctness. |
