...
| [`PolyFun/PFunctor/Dynamical/Machine.lean`](../../PolyFun/PFunctor/Dynamical/Machine.lean) | Pointed machines (`init` / partial `output`) over an interface; `seqComp` (⊕-state sequential composition, Ex 6.41) with `toComp_seqComp_inl/inr`; fuelled `toComp`; monad-parametric `runWith` = `mapM ∘ toComp` with `runWith_succ` / `runWith_output_some` (fuel irrelevance). |
